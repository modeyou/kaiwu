...
def _add_episode_group(monitor, group_name, group_name_en, prefix):
    ...
    metrics = [
        ...
        ("Pre_BuffGain", f"{p}pre_buff_gain"),
        ("Post_BuffGain", f"{p}post_buff_gain"),
        ...
        ("BuffApproachMean", f"{p}buff_approach_mean"),
        ("DangerPenaltyMean", f"{p}danger_penalty_mean"),
        ("FlashRewardMean", f"{p}flash_reward_mean"),
        ("FlashPenaltyMean", f"{p}flash_penalty_mean"),
        ("Direct_Mon1Nearest", f"{p}direct_mon1_nearest_mean"),
        ("Direct_ThreatDanger", f"{p}direct_threat_danger_mean"),
